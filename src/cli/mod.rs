            ...
            if !report_differences.is_empty() {
                for entry in report_differences.iter() {
                    match entry {
                        ...
                        Addition { path, thing: Constraint(name) } => println!("{}: constraint '{}' added", CLI::render_path(path), name),
                        ...
                        Removal { path, thing: Constraint(name) } => println!("{}: constraint '{}' removed", CLI::render_path(path), name),
